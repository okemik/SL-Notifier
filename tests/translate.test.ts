...
import { createTranslator, createGoogleCloudTranslator } from "../src/translate.js";
...
test("Google Cloud translator caches and expires translations", async () => {
  let calls = 0, time = 0;
  const translate = createGoogleCloudTranslator({
    apiKey: "test-key", now: () => time, request: async () => { calls++; return "English"; },
  });
  assert.equal(await translate("Svenska"), "English");
  assert.equal(await translate("Svenska"), "English");
  assert.equal(calls, 1);
...
test("Google Cloud translator rejects oversized input and invalid responses without caching", async () => {
  let calls = 0;
  const translate = createGoogleCloudTranslator({
    apiKey: "test-key", request: async () => { calls++; return "  "; },
  });
  await assert.rejects(translate("x".repeat(4001))); assert.equal(calls, 0);
  await assert.rejects(translate("Hej")); assert.equal(calls, 1);
  await assert.rejects(translate("Hej")); assert.equal(calls, 2);
